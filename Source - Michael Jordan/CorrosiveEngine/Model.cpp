...
void Model::Draw(Camera* camera, Program* program, Transform transform) const
{
	for (Mesh* mesh : m_components)
	{
		mesh->SetColour(m_colour);
...
void Model::ProcessNode(aiNode* node, const aiScene* scene)
{
	...
	for (GLuint i = 0; i < node->mNumMeshes; i++)
	{
		...
		this->m_components.push_back(this->ProcessMesh(mesh, scene));
	}
...
GLint Model::TextureFromFile(const char* path, string directory)
{
	...
	if (image)
	{
		LogManager::GetInstance().Log(("Texture loaded filename: " + filename).c_str());
	...
	LogManager::GetInstance().LogErr(("Failed to load texture filename: " + filename).c_str());
	return 0;
}