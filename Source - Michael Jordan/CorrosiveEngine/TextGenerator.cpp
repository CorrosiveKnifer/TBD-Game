...
void TextGenerator::RenderText(std::string text, glm::vec2 position)
{
	...
	m_renderProgram->SetMatrix4("ProjectionMat", Renderer::SCREEN_PROJ);
	
	m_renderProgram->BindVertextArray(m_VAO);
	...
	for (std::string::const_iterator iter = text.begin(); iter != text.end(); iter++)
	{
		...
		m_renderProgram->SetInteger("TextTexture", 0);
	...
	m_renderProgram->Clear();
}

//	GetDimentionsOfText( _text )
//
//	Author: Michael Jordan
//	Access: private
//	Description:
//		Loops through the text to calculate the width and height of the text 
//		being displayed.
//		
//	Param:
//		- std::string 	|	Text to generate.
//
//	Return: glm::vec2	|	Size of the text being rendered.	
//
glm::vec2 TextGenerator::GetDimentionsOfText(std::string _text)
{
	std::string::iterator iter = _text.begin();
	float xSize = 0.0f;

	while (iter != _text.end())
	{	
		//Get next Character
		Character* glyph = &m_characters[*iter];
		xSize += glyph->Size.x + glyph->Bearing.x;
		iter++;
	}
	
	return glm::vec2(xSize, m_fontSize);
}
